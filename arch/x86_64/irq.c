...
void interrupt_init(void)
{
    ...
    /* pic_init() masks every IRQ; re-enable any handlers that were
     * registered before the controller was switched to PIC mode
     * (e.g. the timer IRQ0 from arch_timer_init). */
...
void irq_init(void)
{
    static const uint64_t stubs[IRQ_COUNT] = {
        (uint64_t)irq0,  (uint64_t)irq1,  (uint64_t)irq2,
        (uint64_t)irq3,  (uint64_t)irq4,  (uint64_t)irq5,
        (uint64_t)irq6,  (uint64_t)irq7,  (uint64_t)irq8,
        (uint64_t)irq9,  (uint64_t)irq10, (uint64_t)irq11,
        (uint64_t)irq12, (uint64_t)irq13, (uint64_t)irq14,
        (uint64_t)irq15,
    };

    for (int i = 0; i < IRQ_COUNT; i++) {
        idt_set_gate(0x20 + i, stubs[i],
